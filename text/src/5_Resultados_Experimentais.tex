...
DINOv2-LoRA com Descongelamento Completo (CV)         & 0{,}814 $\pm$ 0{,}007 & 0{,}784 $\pm$ 0{,}009 & 0{,}013 $\pm$ 0{,}002 & 0{,}102 $\pm$ 0{,}009 & 0{,}469 $\pm$ 0{,}018 \\
AdaFace-Cosseno (CV; sem treino)                      & 0{,}799 $\pm$ 0{,}000 & 0{,}809 $\pm$ 0{,}000 & \textbf{0{,}071 $\pm$ 0{,}000} & \textbf{0{,}218 $\pm$ 0{,}000} & 0{,}524 $\pm$ 0{,}000 \\
Modelo com Recuperação (CV)                           & 0{,}778 $\pm$ 0{,}008 & 0{,}741 $\pm$ 0{,}015 & 0{,}014 $\pm$ 0{,}002 & 0{,}078 $\pm$ 0{,}019 & 0{,}384 $\pm$ 0{,}030 \\
\hline
\end{tabular}
\end{table}
...
A validação cruzada também mostra que algumas execuções únicas do ciclo de desenvolvimento caíram em regiões favoráveis da distribuição. No DINOv2-LoRA, a execução única com AUC 0,822 ficou acima da média por dobra da configuração correspondente (0,814 $\pm$ 0,007). A execução única da configuração Simétrica do AdaFace-Regional, com AUC 0,879, ficou cerca de 1,5 desvio acima da média por dobra dessa mesma configuração (0,873). A execução única da configuração com Negativos Difíceis, com AUC 0,882, também ficou acima da média por dobra (0,876). Já o Modelo com Recuperação teve execução única praticamente coincidente com a validação cruzada, com AUC 0,776 contra média 0,778 $\pm$ 0,008. Esse resultado confirma a necessidade de reportar médias por dobra, especialmente quando as diferenças entre configurações são pequenas.

Sob validação cruzada, o DINOv2-LoRA alcança AUC 0,814 $\pm$ 0,007, ficando acima da linha de base AdaFace-Cosseno em AUC, mas abaixo do ViT-FaCoR. O resultado indica que a adaptação do DINOv2 por LoRA e descongelamento completo melhora a linha de base sem treinamento na ordenação global, porém não alcança a arquitetura com ViT e atenção cruzada bidirecional. O Modelo com Recuperação apresenta AUC 0,778 $\pm$ 0,008, abaixo do AdaFace-Cosseno, mas ainda é relevante na análise por relação porque o mecanismo de recuperação tende a reduzir a diferença entre classes frequentes e classes raras. Dessa forma, DINOv2-LoRA e Modelo com Recuperação ajudam a delimitar duas hipóteses negativas: a representação auto-supervisionada adaptada não resolve sozinha o problema, e a recuperação de exemplos melhora a uniformidade em algumas relações sem necessariamente melhorar o ranking global.
...
A leitura do DINOv2-LoRA é negativa para a hipótese de que DINOv2 congelado com LoRA e atenção diferencial seria suficiente para alcançar o ViT-FaCoR. Mesmo na configuração com maior AUC global em execução única, a validação cruzada posterior fica em 0,814 $\pm$ 0,007 AUC, abaixo do ViT-FaCoR. Esse ganho em AUC ocorre, porém, em detrimento do regime de baixa falsa aceitação, que apresenta degradação em relação à configuração inicial com LoRA. A variante híbrida com pesos do ViT-FaCoR também não transfere o desempenho do sistema original, o que sugere que o resultado do ViT-FaCoR depende do conjunto formado por backbone, atenção cruzada, função de perda e calibração, e não apenas do encoder isolado.
...
Sob validação cruzada, o AdaFace-Regional obteve o maior AUC entre as arquiteturas avaliadas em FIW. O resultado da configuração com Negativos Difíceis é 0,876 $\pm$ 0,003 AUC, acima do ViT-FaCoR (0,846 $\pm$ 0,004), do DINOv2-LoRA (0,814 $\pm$ 0,007), da linha de base AdaFace-Cosseno (0,799 $\pm$ 0,000) e do Modelo com Recuperação (0,778 $\pm$ 0,008). A principal evidência do capítulo é, portanto, que a proposta baseada em AdaFace parcialmente descongelado, tokens regionais anatômicos, cabeça auxiliar de relação e passagem simétrica melhora a verificação de parentesco no FIW em relação às alternativas avaliadas dentro do protocolo adotado.

Fora da comparação principal em validação cruzada, os VLMs foram avaliados como linhas de base externas zero-shot. Nesse cenário, o Claude Sonnet binário alcança 0,789 AUC e o Codex VLM binário alcança 0,624 AUC, ambos sem validação cruzada.
