...
Este capítulo apresenta os resultados experimentais obtidos no benchmark Families in the Wild (FIW). A comparação principal segue o protocolo descrito no Capítulo~\ref{cap:Metodologia}, com validação cruzada de cinco dobras separadas por família, escolha do limiar apenas na validação e avaliação no conjunto de teste do RFIW Track-I. Entram nessa comparação a linha de base AdaFace-Cosseno, o ViT-FaCoR, o DINOv2-LoRA, o Modelo com Recuperação e as três configurações finalistas do AdaFace-Regional, denominadas Simétrica, Comparativa e com Negativos Difíceis. Os VLMs são apresentados em seção própria como linhas de base externas zero-shot, avaliadas em amostras balanceadas de 6.000 pares e sem validação cruzada.
...
Average Precision & \textbf{0{,}882} & 0{,}742 & +0{,}140 \\
...
No manifesto usado pelo Claude, o AdaFace-Regional R012 supera o Claude Sonnet em todas as métricas reportadas. A diferença é especialmente grande em AUC, Average Precision e TAR@FAR=0{,}01, indicando que a principal limitação do VLM zero-shot está na ordenação dos pares e na operação em baixa falsa aceitação. Como esse manifesto é balanceado e tem negativos construídos de forma diferente do teste oficial completo, o AUC de 0,888 dessa avaliação deve ser lido como resultado específico desse subconjunto, não como novo resultado principal do AdaFace-Regional.
...
Fora da comparação principal em validação cruzada, os VLMs foram avaliados como linhas de base externas zero-shot. Nesse cenário, o Claude Sonnet binário alcança AUC de 0,789 e o Codex VLM binário alcança AUC de 0,624, ambos sem validação cruzada. No mesmo manifesto de 6.000 pares usado pelo Claude, o AdaFace-Regional R012 alcança AUC de 0,888, reforçando que a diferença em relação ao VLM permanece quando o conjunto de pares é controlado.
