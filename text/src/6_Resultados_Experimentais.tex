...
Este capítulo apresenta os resultados experimentais obtidos no \textit{benchmark} Families in the Wild (FIW). A comparação principal segue o protocolo descrito no Capítulo~\ref{cap:Metodologia}, com validação cruzada de cinco dobras separadas por família, escolha do limiar apenas na validação e avaliação no conjunto de teste do RFIW Track-I. Entram nessa comparação a linha de base AdaFace-Cosseno, o ViT-FaCoR, o DINOv2-LoRA, o Modelo com Recuperação e as três configurações finalistas do AdaFace-Regional, denominadas Simétrica, Comparativa e com Negativos Difíceis. Os VLMs são apresentados em seção própria como linhas de base externas \textit{zero-shot}, avaliadas em amostras balanceadas de 6.000 pares e sem validação cruzada.

A métrica principal é o ROC AUC, por ser independente do limiar de decisão. \textit{Average Precision} e TAR@FAR complementam essa leitura ao avaliar, respectivamente, a curva precisão-revocação e os regimes de baixa falsa aceitação. A acurácia é usada principalmente na análise por tipo de relação, porque permite observar quais vínculos familiares continuam mais difíceis para cada configuração.
...
\hline
\textbf{Regime} & \textbf{Params} & \textbf{AUC} & \textbf{Acc.} & \textbf{TAR@0{,}01} & \textbf{\textit{Gap} val-teste} \\
\hline
\textit{LoRA-only}                     & 8{,}5M  & 0{,}806 & 72{,}6\% & \textbf{0{,}152} & -0{,}105 \\
regularização mais forte      & 8{,}5M  & 0{,}799 & 72{,}4\% & 0{,}095          & -0{,}106 \\
\textit{sampler} estratificado         & 8{,}5M  & 0{,}809 & 71{,}0\% & 0{,}098          & -0{,}100 \\
...
A leitura do DINOv2-LoRA é desfavorável à hipótese de que DINOv2 congelado com LoRA e atenção diferencial seria suficiente para alcançar o ViT-FaCoR. Entre as execuções de desenvolvimento, o melhor AUC global aparece no descongelamento completo, enquanto a configuração inicial com LoRA preserva o melhor TAR@FAR=0{,}01. Esse comportamento mostra que o ganho em AUC ocorre em detrimento do regime de baixa falsa aceitação. A variante híbrida com pesos do ViT-FaCoR também não transfere o desempenho do sistema original, o que sugere que o resultado do ViT-FaCoR depende do conjunto formado por \textit{backbone}, atenção cruzada, função de perda e calibração, e não apenas do \textit{encoder} isolado.
...
\begin{tabularx}{\textwidth}{Xrrrrr}
\hline
\textbf{Intervenção principal} & \textbf{AUC} & \textbf{AP} & \textbf{TAR@0{,}001} & \textbf{TAR@0{,}01} & \textbf{\textit{Gap} val-teste} \\
\hline
...
No ciclo de desenvolvimento, duas mudanças aparecem associadas aos maiores saltos de desempenho. A primeira é o descongelamento parcial. Com AdaFace congelado, o modelo fica em AUC de 0,746, enquanto o descongelamento parcial sobe para 0,856 ao liberar o último estágio e a camada de saída. A segunda é a passagem simétrica, que processa cada par nas duas ordens, $(A,B)$ e $(B,A)$, e reduz a dependência da posição das imagens. Essa mudança aumentou o AUC no ciclo de desenvolvimento em 0,022 em relação ao descongelamento parcial e reduziu o \textit{gap} validação-teste de -0,076 para -0,026.
...
\hline
\textit{non-kin}             & 6.993 & 67{,}5 $\pm$ 2{,}8 & \textbf{69{,}7 $\pm$ 4{,}4} & \textbf{69{,}7 $\pm$ 1{,}8} \\
\hline
\end{tabular}
\end{table}
...
A classe \textit{non-kin} apresenta o movimento oposto. A configuração Simétrica é a mais forte nas classes positivas, mas tem menor acurácia em não-parentes, com 67,5 $\pm$ 2,8. As configurações Comparativa e com Negativos Difíceis sobem para 69,7, indicando maior especificidade. Esse comportamento é coerente com as métricas TAR@FAR, pois configurações que rejeitam melhor pares não aparentados tendem a perder parte da acurácia em relações positivas. A escolha entre esses regimes depende do custo relativo entre falso positivo e falso negativo na aplicação.

\section{Linhas de base com modelos multimodais}

Os VLMs foram avaliados novamente na formulação binária \textit{kin}/\textit{non-kin}, para alinhar a pergunta com a dos modelos supervisionados. Cada modelo recebeu 6.000 pares balanceados do FIW, com 3.000 positivos e 3.000 negativos, em regime \textit{zero-shot}. A Tabela~\ref{tab:vlm_binario_resultados} resume as métricas globais, usando o escore contínuo derivado da decisão e da confiança conforme a Equação~\ref{eq:vlm_score}.

\begin{table}[htbp]
\centering
\caption{Resultados dos VLMs em verificação binária \textit{zero-shot} no FIW, com 6.000 pares balanceados por modelo.}
...
\hline
AUC & 0{,}789 & 0{,}624 \\
\textit{Average Precision} & 0{,}742 & 0{,}592 \\
...
O Claude Sonnet apresenta desempenho acima do acaso e fica próximo da linha de base AdaFace-Cosseno em AUC, mas ligeiramente abaixo dela. O valor é 0,789 contra 0,799. A acurácia de 72,3\% e o F1 de 0,744 mostram que o modelo consegue responder à tarefa binária de forma coerente em muitos casos, porém a \textit{Average Precision} e o TAR@FAR=0{,}01 indicam limitação forte de ordenação. A distribuição de confiança é discreta, com poucos valores recorrentes, e isso reduz a utilidade do modelo em regimes de falsa aceitação estrita.

O Codex VLM tem comportamento mais enviesado para a classe positiva. Sua revocação para \textit{kin} é 0,799, parecida com a do Claude, mas a especificidade cai para 0,381. Em termos absolutos, isso significa que 1.857 dos 3.000 pares \textit{non-kin} foram classificados como parentes. A matriz de confusão da Tabela~\ref{tab:vlm_confusao_binaria} explicita esse padrão. O resultado sugere que, em \textit{zero-shot}, o Codex apresenta viés de aceitação positiva, com baixa especificidade diante de pares não biológicos com semelhança visual fortuita.
...
Para comparar o VLM mais forte com uma referência supervisionada no mesmo conjunto de pares, o \textit{checkpoint} R012 do AdaFace-Regional foi executado no manifesto de 6.000 pares usado pelo Claude Sonnet. Essa avaliação usa exatamente os pares da execução do Claude, mas não usa o teste completo de 13.425 pares do protocolo RFIW Track-I. Portanto, os resultados da Tabela~\ref{tab:m12_claude_manifest} servem apenas para a comparação controlada com esse VLM, não para substituir o resultado principal em validação cruzada.

\begin{table}[htbp]
\centering
\caption{Comparação entre AdaFace-Regional R012 e Claude Sonnet no mesmo manifesto de 6.000 pares.}
\label{tab:m12_claude_manifest}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{0.92\textwidth}{Xccc}
\hline
\textbf{Métrica} & \textbf{M12 R012} & \textbf{Claude} & \textbf{$\Delta$} \\
\hline
AUC & \textbf{0{,}888} & 0{,}789 & +0{,}100 \\
\textit{Average Precision} & \textbf{0{,}882} & 0{,}742 & +0{,}140 \\
TAR@0{,}001 & \textbf{0{,}066} & 0{,}042 & +0{,}024 \\
TAR@0{,}01 & \textbf{0{,}259} & 0{,}042 & +0{,}217 \\
TAR@0{,}1 & \textbf{0{,}634} & 0{,}297 & +0{,}336 \\
Acurácia & \textbf{0{,}800} & 0{,}723 & +0{,}077 \\
Precisão & \textbf{0{,}755} & 0{,}691 & +0{,}064 \\
Revocação & \textbf{0{,}888} & 0{,}806 & +0{,}082 \\
Especificidade & \textbf{0{,}712} & 0{,}639 & +0{,}072 \\
F1 & \textbf{0{,}816} & 0{,}744 & +0{,}072 \\
\hline
\end{tabularx}
\end{table}

No manifesto usado pelo Claude, o AdaFace-Regional R012 supera o Claude Sonnet em todas as métricas reportadas. A diferença é especialmente grande em AUC, \textit{Average Precision} e TAR@FAR=0{,}01, indicando que a principal limitação do VLM \textit{zero-shot} está na ordenação dos pares e na operação em baixa falsa aceitação. Como esse manifesto é balanceado e tem negativos construídos de forma diferente do teste oficial completo, o AUC de 0,888 dessa avaliação deve ser lido como resultado específico desse subconjunto, não como novo resultado principal do AdaFace-Regional.

A comparação com os modelos supervisionados deve ser feita com cautela. Os VLMs respondem à mesma tarefa binária, mas não têm treinamento no FIW, não usam validação cruzada e não ajustam limiar por validação. Ainda assim, as métricas independentes de limiar são informativas. O Claude fica próximo da linha de base facial sem treinamento, mas abaixo dela; o Codex fica substancialmente abaixo; e a comparação pareada no manifesto usado pelo Claude mostra uma vantagem clara do AdaFace-Regional R012 sobre o VLM mais forte. Isso indica que, neste protocolo, conhecimento visual geral não substitui adaptação supervisionada para verificação de parentesco facial.

A avaliação multiclasse anterior permanece como diagnóstico histórico. Nela, o Codex \textit{zero-shot} alcançou 33,1\% de acurácia e macro-F1 de 0,257 em 750 pares positivos; a adaptação por \textit{prompt} caiu para 26,7\% de acurácia e macro-F1 de 0,223; e o piloto do Claude Sonnet em 75 pares ficou em torno de 37\% de acurácia. Como esse protocolo não inclui \textit{non-kin}, ele não é usado na comparação principal. O recorte de 12 pares de avós e netos com \textit{prompts} direcionados mostrou que orientar explicitamente o modelo para relações de duas gerações ajuda a escolher o grupo correto, mas quase não resolve a identificação da relação exata.
