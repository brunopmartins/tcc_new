...
No parentesco facial, os VLMs entram pela possibilidade de testar a tarefa sem treinamento específico no FIW. O procedimento é simples: envia-se uma imagem com duas faces e um prompt descrevendo as relações possíveis. A resposta do modelo passa a depender das informações extraídas da imagem e da instrução textual. Essa simplicidade, porém, também limita a leitura do resultado. Como o modelo não foi ajustado para relações genealógicas finas, seu desempenho pode ser influenciado por variáveis de confusão mais imediatas, como idade aparente, gênero ou semelhança geral entre as faces.
