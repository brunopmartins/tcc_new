...
No parentesco facial, os VLMs entram pela possibilidade de testar a tarefa sem treinamento específico no FIW. O procedimento é simples: envia-se uma imagem com duas faces e um prompt descrevendo as relações possíveis. A resposta do modelo passa a depender do que ele consegue extrair da imagem e da instrução textual. Essa simplicidade, porém, também limita a leitura do resultado. Como o modelo não foi ajustado para relações genealógicas finas, ele pode se apoiar em pistas mais imediatas, como idade aparente, gênero ou semelhança geral entre as faces.
...
A acurácia mede a fração total de decisões corretas. Ela é intuitiva, mas pode esconder problemas quando uma classe é mais difícil que outra. A precisão mede, entre os pares classificados como parentes, quantos realmente eram positivos. A revocação mede, entre os pares que eram parentes, quantos foram recuperados pelo modelo. O F1 combina precisão e revocação por média harmônica, sendo útil quando se quer observar os dois efeitos em uma única medida. Neste trabalho, essas métricas dependem do limiar $\tau^*$ escolhido na validação.
...
Average Precision também é independente de um único limiar, mas olha para a relação entre precisão e revocação. Ela resume a curva precisão-revocação e é útil quando interessa saber se os pares positivos aparecem bem ranqueados entre os maiores escores. No contexto deste trabalho, ela complementa o AUC porque avalia a qualidade do ranking por outro ponto de vista.
...
Para os VLMs, a tarefa é diferente: eles escolhem uma entre onze relações positivas, sem classe non-kin. Por isso, são usadas métricas multiclasse. A acurácia global mede a fração de respostas corretas. Macro-precision, macro-recall e macro-F1 calculam a métrica separadamente por classe e depois tiram a média, sem dar mais peso às classes maiores. A matriz de confusão completa a análise porque mostra para quais relações o modelo desvia quando erra, por exemplo se confunde avô-neto com pai-filho.

Com essas definições, a leitura dos resultados fica menos dependente de uma única medida. O AUC e a Average Precision avaliam a ordenação geral dos pares, as métricas com limiar mostram o comportamento no ponto escolhido em validação, e o TAR@FAR mostra o desempenho quando a falsa aceitação é controlada explicitamente. Nos VLMs, as métricas macro e a matriz de confusão ajudam a enxergar o comportamento por classe, já que a tarefa é multiclasse e não verificação binária.
