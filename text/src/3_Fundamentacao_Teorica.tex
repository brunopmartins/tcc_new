...
Em contraste com backbones de uso geral, modelos especializados em reconhecimento facial usam arquiteturas e perdas voltadas à separação de identidades em large-scale. O AdaFace \cite{kim2022adaface}, baseado em uma variante IR-101 da família ResNet adaptada para faces, é treinado em WebFace4M com uma loss de margem adaptativa que ajusta o peso de exemplos difíceis em função da qualidade da imagem. O resultado é um extrator de 512 dimensões em que distâncias entre embeddings refletem similaridade de identidade. No Modelo 12 deste trabalho, o AdaFace IR-101 entra como backbone facial especializado, com partial unfreeze do último estágio, e seus embeddings alimentam a estrutura regional descrita no Capítulo~\ref{cap:Metodologia}.
...
Uma variante recente reduz drasticamente o número de tokens envolvidos na cross-attention ao trocar patches densos por regiões anatômicas fixas. Em vez de 196 tokens espaciais como no ViT base, define-se um pequeno conjunto de regiões da face (por exemplo, global, olhos, nariz, boca e mandíbula) e cada região vira um único token. A cross-region attention opera sobre uma matriz pequena, da ordem de cinco por cinco no caso anatômico, e fornece um prior estrutural ao restringir explicitamente onde o modelo compara as duas faces. O Modelo 12 deste trabalho usa essa abordagem regional combinada com um mecanismo de gate sigmoide que aprende pesos por região, permitindo que múltiplas regiões sejam salientes ao mesmo tempo.
