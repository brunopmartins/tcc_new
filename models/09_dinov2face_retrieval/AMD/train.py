#!/usr/bin/env python3
"""
Model 09 — DINOv2-Face + Retrieval-Augmented Kinship, AMD ROCm training.

Adapted from Model 08 train.py. The only architectural difference is the
encoder: DINOv2 ViT-B/14 (frozen, optionally overlaid with face-pretrained
state_dict) instead of ArcFace IResNet-100. Input is 224×224 with ImageNet
normalization — DINOv2's native pretraining resolution and normalization.

Loss, optimization, gallery handling, and trainer are identical to M08 / M06.
"""
...
from model import DINOv2FaceRetrievalKinship, RELATION_SETS  # noqa: E402


# DINOv2 was pretrained with ImageNet normalization at 224×224 resolution.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Model 09 (AMD ROCm)")
    ...
    p.add_argument("--data_root", type=str, default=None,
                   help="Override dataset root (where track-I/ CSVs live)")
    p.add_argument("--aligned_root", type=str, default=None,
                   help="Path to pre-aligned face crops; remaps paths at load "
                        "time without changing pair-construction logic. "
                        "Convention: ALIGNED_ROOT env var.")
    p.add_argument("--num_workers", type=int, default=4)

    # Encoder — DINOv2 / DINOv2-Face
    p.add_argument("--dinov2_weights", type=str,
                   default="",
                   help="Path to a DINOv2-Face state_dict to overlay on top of "
                        "the timm DINOv2 base. Empty string ('') = use base "
                        "DINOv2 only (no face fine-tune). See weights/README.md "
                        "for download options.")
    p.add_argument("--dinov2_model_name", type=str,
                   default="vit_base_patch14_dinov2.lvd142m",
                   help="timm model identifier for the backbone. The base "
                        "DINOv2 ViT-B/14 is the default.")
    p.add_argument("--strict_load", action="store_true",
                   help="Fail if the DINOv2-Face overlay has any "
                        "missing/unexpected keys.")
    p.add_argument("--pool", type=str, default="cls", choices=["cls", "mean"],
                   help="Token pooling strategy for the DINOv2 ViT.")
    p.add_argument("--img_size", type=int, default=DINOV2_IMG_SIZE,
                   help="Input resolution. Must be divisible by patch size 14 "
                        "for DINOv2 (224, 238, 252, ...). Default 224.")
    ...
    p.add_argument("--batch_size", type=int, default=8,
                   help="DINOv2 ViT-B/14 at 224×224 is larger than ArcFace 112 "
                        "— start with batch 8 (M06 default).")
...
class RetrievalLoss(nn.Module):
    """BCE + optional contrastive + relation-CE. Same as M06/M08."""
    def __init__(self, num_relations: int, loss_type: str,
                 contrastive_weight: float, relation_weight: float,
                 temperature: float):
        super().__init__()
    ...
    def forward(self, output, labels, relation_indices):
        ...
        if valid.any():
            loss_rel = self.rel_ce(output["relation_logits"][valid],
                                    relation_indices[valid]).mean()
        else:
...
class RetrievalROCmTrainer(ROCmTrainer):
    """Same trainer as M06/M08 — gradient accumulation + clear cache + gallery refresh hook."""
    def __init__(self, *args, gradient_accumulation: int = 1,
                 relation_to_index=None, gallery_builder=None,
                 gallery_refresh_every: int = 0, **kwargs):
...
def main() -> None:
    args = parse_args()
    print("\n" + "=" * 60)
    print("Model 09 — DINOv2-Face + Retrieval-Augmented Kinship | AMD ROCm")
    print("=" * 60)
    ...
    # Configure data — DINOv2-style ImageNet normalization at 224×224.
    train_data_config = DataConfig(
        split_seed=args.seed,
        image_size=args.img_size,
        normalize_mean=DINOV2_MEAN,
        normalize_std=DINOV2_STD,
    )
    ...
    print(f"Image size:    {args.img_size}  (ImageNet normalization)")
    print(f"Backbone:      {args.dinov2_model_name}")
    print(f"DINOv2 overlay:{args.dinov2_weights or '(none — using base timm weights)'}")
    ...
    print("\nBuilding Model 09...")
    print(f"  encoder:     DINOv2 ViT-B/14 (face-overlay={'yes' if args.dinov2_weights else 'no'})")
    print(f"  K retrievals:{args.retrieval_k}")
    print(f"  relation set:{relation_set}")

    model = DINOv2FaceRetrievalKinship(
        dinov2_weights=args.dinov2_weights or None,
    ...
    # Gallery loader: training positives only, no shuffle.
