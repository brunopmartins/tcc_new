#!/usr/bin/env python3
"""Model 09 — AMD ROCm full analysis (DINOv2-Face + retrieval)."""
...
from model import DINOv2FaceRetrievalKinship  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate Model 09 (AMD ROCm)")
...
def build_from_checkpoint(ckpt) -> DINOv2FaceRetrievalKinship:
    cfg = ckpt.get("model_config", ckpt.get("protocol", {}).get("model_config", {}))
    return DINOv2FaceRetrievalKinship(
        dinov2_weights=cfg.get("dinov2_weights"),
        dinov2_model_name=cfg.get("dinov2_model_name",
                                   "vit_base_patch14_dinov2.lvd142m"),
        img_size=cfg.get("img_size", DINOV2_IMG_SIZE),
...
def main() -> None:
    ...
    if args.full_analysis:
        ...
        plt.xlabel("FPR"); plt.ylabel("TPR"); plt.title("ROC — Model 09")
        ...
        plt.title("Confusion — Model 09"); plt.tight_layout()
        ...
        if per_rel:
            ...
            ax.set_ylim(0, 1)
            ax.set_title("Per-relation accuracy — Model 09")
