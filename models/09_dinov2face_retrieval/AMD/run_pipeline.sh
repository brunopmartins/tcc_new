#!/bin/bash
# =============================================================================
# Model 09 — DINOv2-Face + Retrieval-Augmented Kinship — AMD ROCm pipeline
#
# End-to-end local runner: venv setup → train → test → evaluate. The encoder
# is a frozen DINOv2 ViT-B/14 (timm), optionally overlaid with face-pretrained
# weights ("DINOv2-Face"). If no DINOv2-Face weights are available, the
# pipeline falls back to base DINOv2 — see weights/README.md.
#
# Usage:
...
#   EPOCHS                20
#   BATCH_SIZE            8           (DINOv2 224×224 is larger than ArcFace)
...
#   SCHEDULER             cosine
#   TRAIN_DATASET         fiw
#   DINOV2_WEIGHTS        ""          (empty = use base DINOv2 from timm)
#   DINOV2_MODEL_NAME     vit_base_patch14_dinov2.lvd142m
#   POOL                  cls         (or 'mean')
#   IMG_SIZE              224
#   STRICT_LOAD           0           (1 = fail on key mismatch)
#   RETRIEVAL_K           32
...
DINOV2_MODEL_NAME="${DINOV2_MODEL_NAME:-vit_base_patch14_dinov2.lvd142m}"
POOL="${POOL:-cls}"
...
if [ -e /dev/kfd ] && [ -z "${_ROCM_SETUP_COMPLETE}" ]; then
    ...
    export TRAIN_DATASET DINOV2_WEIGHTS DINOV2_MODEL_NAME POOL IMG_SIZE STRICT_LOAD
...
# ---------- preconditions ------------------------------------------------------
if [ -n "${DINOV2_WEIGHTS}" ] && [ ! -f "${DINOV2_WEIGHTS}" ]; then
    echo "ERROR: DINOV2_WEIGHTS set to ${DINOV2_WEIGHTS} but file not found"
    echo "Either:"
    echo "  - download DINOv2-Face weights to that path (see weights/README.md)"
    echo "  - unset DINOV2_WEIGHTS to fall back to base DINOv2 from timm"
    exit 1
fi
...
echo '============================================'
echo 'Model 09: DINOv2-Face + Retrieval-Augmented Kinship'
...
echo "Backbone:          ${DINOV2_MODEL_NAME} (frozen)"
echo "DINOv2 overlay:    ${DINOV2_WEIGHTS:-(none — base timm weights)}"
...
[ "${STRICT_LOAD}" = "1" ]          && TRAIN_ARGS+=(--strict_load)
