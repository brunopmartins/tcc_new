...
UNFREEZE_FACE_BACKBONE_BLOCKS="${UNFREEZE_FACE_BACKBONE_BLOCKS:-0}"
GPU_ID="${GPU_ID:-0}"
SEED="${SEED:-42}"
...
if [ -e /dev/kfd ] && [ -z "${_ROCM_SETUP_COMPLETE}" ]; then
    ...
    export USE_HYBRID_BACKBONE FACE_BACKBONE_NAME FACE_BACKBONE_CHECKPOINT FACE_BACKBONE_STATE_PREFIX INTRA_FACE_ATTN_LAYERS UNFREEZE_FACE_BACKBONE_BLOCKS
...
if [ "${USE_HYBRID_BACKBONE}" = "1" ]; then
    ...
    [ "${UNFREEZE_FACE_BACKBONE_BLOCKS}" != "0" ] && TRAIN_ARGS+=(--unfreeze_face_backbone_blocks "${UNFREEZE_FACE_BACKBONE_BLOCKS}")
fi
