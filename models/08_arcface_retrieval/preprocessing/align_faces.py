"""
MTCNN face alignment + 112×112 crop preprocessing for ArcFace.

Reads the FIW (or KinFaceW) image set under the configured dataset root,
detects + aligns each face with MTCNN, saves the 112×112 ArcFace-style crop
to a parallel directory tree.

Designed to run once. Subsequent training reads from the cached aligned dir.

Usage:
    python align_faces.py --dataset fiw \\
        --src-root datasets/FIW \\
        --dst-root models/08_arcface_retrieval/data/fiw_aligned
...
def parse_args():
    ...
    p.add_argument("--dst-root", required=True,
                   help="Where to write 112×112 aligned crops")
    p.add_argument("--rocm-device", type=int, default=0)
    p.add_argument("--batch-size", type=int, default=32,
                   help="MTCNN inference batch size")
...
def align_face(img: Image.Image, landmarks: np.ndarray, output_size: int = 112) -> Image.Image:
    """Apply similarity transform mapping detected landmarks to ARCFACE_TEMPLATE."""
    import cv2
    img_np = np.array(img)
    T = umeyama_similarity(landmarks.astype(np.float32), ARCFACE_TEMPLATE)
    M = T[:2, :]
...
def fallback_center_crop(img: Image.Image, output_size: int = 112) -> Image.Image:
...
def main():
    ...
    for src_path, rel_path in tqdm(images, desc="Aligning"):
        ...
        if landmarks is not None and len(landmarks) > 0:
            aligned = align_face(img, landmarks[0], output_size=112)
        else:
            aligned = fallback_center_crop(img, output_size=112)
            n_fallback += 1
