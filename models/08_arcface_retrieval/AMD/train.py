#!/usr/bin/env python3
"""
Model 08 — ArcFace + Retrieval-Augmented Kinship, AMD ROCm training.

Adapted from Model 06 train.py. The only architectural difference is the
encoder: ArcFace IResNet-100 (frozen, MS1MV2 pretrained) instead of timm
ViT/DINOv2. Input is 112×112 MTCNN-aligned crops (preprocessed offline).

Loss, optimization, gallery handling, and trainer are identical to M06 R001.
"""
...
from model import ArcFaceRetrievalKinship, RELATION_SETS  # noqa: E402


# ArcFace expects inputs normalized to [-1, 1] (NOT ImageNet mean/std).
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Model 08 (AMD ROCm)")
    ...
    # Encoder — ArcFace
    p.add_argument("--arcface_weights", type=str,
                   default=str(Path(__file__).parent.parent / "weights" / "arcface_r100.pth"),
                   help="Path to ArcFace weights. Accepts two formats: (a) state "
                        "dict for the local iresnet50/100 class, or (b) full "
                        "pickled nn.Module (e.g. onnx2torch GraphModule from a "
                        "converted InsightFace .onnx). Default: R100 Glint360K "
                        "from converting lithiumice/insightface's glintr100.onnx.")
    p.add_argument("--arcface_arch", type=str, default="r100",
                   choices=["r50", "r100"],
                   help="Only used for format-A (state dict). Ignored for "
                        "format-B (pickled module).")
    p.add_argument("--img_size", type=int, default=ARCFACE_IMG_SIZE)
    ...
    p.add_argument("--batch_size", type=int, default=16,
                   help="ArcFace at 112×112 is small — can fit larger batches")
...
class RetrievalLoss(nn.Module):
    """BCE + optional contrastive + relation-CE. Same as M06."""
...
class RetrievalROCmTrainer(ROCmTrainer):
    """Same trainer as M06 — gradient accumulation + clear cache + gallery refresh hook."""
...
def main() -> None:
    args = parse_args()
    print("\n" + "=" * 60)
    print("Model 08 — ArcFace + Retrieval-Augmented Kinship | AMD ROCm")
    print("=" * 60)
    ...
    # Configure data — ArcFace-specific 112×112 + [-1,1] normalization.
    ...
    print(f"Image size: {args.img_size}  (ArcFace normalization)")
    ...
    print("\nBuilding Model 08...")
    print(f"  encoder:    ArcFace IResNet-{args.arcface_arch.upper().lstrip('R')}")
    print(f"  weights:    {args.arcface_weights}")
    print(f"  K retrievals: {args.retrieval_k}")
    print(f"  relation set: {relation_set}")

    model = ArcFaceRetrievalKinship(
        arcface_weights=args.arcface_weights,
        arcface_arch=args.arcface_arch,
