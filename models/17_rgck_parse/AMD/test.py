#!/usr/bin/env python3
"""AMD ROCm test script for Model 17 — PG-RGCK (parsing-guided regions)."""
import argparse
import os
import sys
...
from model import build_pg_rgck_net as build_rgck_net  # noqa: E402  (M17 parsing-guided)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test Model 17 PG-RGCK (AMD ROCm)")
    ...
    parser.add_argument("--region_box_cache", type=str, default=None,
                        help="M17: per-image region-box .npz (same as training). "
                             "Unset ⇒ static boxes (M15).")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("AMD ROCm Testing — Model 17: PG-RGCK")
    print("=" * 60)
    ...
    model = build_rgck_net(
        ...
        global_token_mode=model_config.get("global_token_mode", "exact"),
    )
    ...
    test_dataset = KinshipPairDataset(
        ...
        region_box_cache=args.region_box_cache,  # M17: per-image boxes at test
    )
    ...
    with open(results_path, "w") as f:
        f.write("AMD ROCm Test Results — Model 17 (PG-RGCK)\n")
        f.write("=" * 50 + "\n")
