#!/usr/bin/env bash
# =====================================================================
# Model 17: PG-RGCK (parsing-guided regions) — AMD ROCm pipeline runner
#
# Usage:
...
ROI_ALIGN_TOKENIZER="${ROI_ALIGN_TOKENIZER:-0}"        # R013: ignored by M15 (M15 always hi-res ROI-Align)
GLOBAL_TOKEN_MODE="${GLOBAL_TOKEN_MODE:-exact}"        # M15: 'exact' (genuine 112 AdaFace embedding) or 'roi' (full-box from hi-res map)
REGION_BOX_CACHE="${REGION_BOX_CACHE:-}"               # M17: .npz of per-image region boxes (tools/parse_faces_boxes.py). Empty ⇒ runs as M15 (fixed boxes).
...
echo '============================================'
echo 'Model 17: PG-RGCK (parsing-guided regions, hi-res ROI-Align)'
echo '============================================'
...
echo "Tokenizer:           hi-res ROI-Align (global_token_mode=${GLOBAL_TOKEN_MODE})"
if [ -n "${REGION_BOX_CACHE}" ]; then
echo "Region boxes:        PER-IMAGE from ${REGION_BOX_CACHE} (geometric aug OFF)"
else
echo "Region boxes:        FIXED (M15 default — set REGION_BOX_CACHE for M17)"
fi
...
TRAIN_ARGS+=(--global_token_mode "${GLOBAL_TOKEN_MODE}")
TRAIN_ARGS+=(--hard_negative_ratio "${HARD_NEGATIVE_RATIO}")
if [ -n "${FOLD}" ]; then
    TRAIN_ARGS+=(--fold "${FOLD}" --num_folds "${NUM_FOLDS}")
fi
# M17: parsing-guided per-image boxes. When set, also disable geometric aug so
# cached boxes stay registered. Unset ⇒ this runs exactly as M15 (fixed boxes).
if [ -n "${REGION_BOX_CACHE}" ]; then
    TRAIN_ARGS+=(--region_box_cache "${REGION_BOX_CACHE}" --no_geometric_aug)
fi
...
if [ -n "${REGION_BOX_CACHE}" ]; then
    TEST_ARGS+=(--region_box_cache "${REGION_BOX_CACHE}")
fi
...
echo ''
echo '============================================'
echo "Model 17 (PG-RGCK) — Run ${RUN_LABEL} completed!"
echo '============================================'
