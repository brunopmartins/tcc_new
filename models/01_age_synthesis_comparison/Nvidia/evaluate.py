#!/usr/bin/env python3
"""
Comprehensive evaluation script for Age Synthesis model.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Age Synthesis Model")
...
def plot_roc_curve(predictions, labels, output_path):
    ...
    plt.title('ROC Curve - Age Synthesis Model')
...
def plot_confusion_matrix(predictions, labels, threshold, output_path):
    ...
    plt.title('Confusion Matrix - Age Synthesis Model')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150)
    plt.close()
...
def plot_per_relation_accuracy(per_relation_metrics, output_path):
    ...
    plt.title('Per-Relation Accuracy - Age Synthesis Model')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150)
    plt.close()
...
def plot_comparison_matrix_analysis(model, dataloader, device, output_path):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Analyzing comparisons"):
    ...
    plt.suptitle('Age Comparison Matrix Analysis')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150)
    plt.close()
...
def run_ablation_study(model, dataloader, device, output_dir):
    ...
    for method in aggregation_methods:
        ...
        # Modify aggregation (if model supports it)
    ...
    ax.set_ylabel('Score')
    ax.set_title('Ablation Study: Aggregation Methods')
    ax.set_xticks(x)
...
def main():
    ...
    if args.full_analysis:
        ...
        plot_roc_curve(predictions, labels, output_dir / "roc_curve.png")
        
        print("Generating confusion matrix...")
        plot_confusion_matrix(predictions, labels, 0.5, output_dir / "confusion_matrix.png")
        
        if "per_relation" in metrics:
            print("Generating per-relation analysis...")
            plot_per_relation_accuracy(metrics["per_relation"], output_dir / "per_relation.png")
        
        print("Analyzing comparison matrix...")
        plot_comparison_matrix_analysis(model, test_loader, device, output_dir / "comparison_analysis.png")
