...
from model import build_fa_rgck_net as build_rgck_net, DEFAULT_REGIONS_224  # noqa: E402  (M16 family-adversarial)
...
def parse_args() -> argparse.Namespace:
    ...
    # M16 family-adversarial (domain generalisation; each FIW family = a domain)
    p.add_argument("--family_adv_weight", type=float, default=0.1,
                   help="Weight of the family-adversarial CE term. 0 disables "
                        "it (model == M12 R011). DANN: the GRL reverses this "
...
class FARGCKROCmTrainer(RGCKROCmTrainer):
    """M16 trainer — adds the family-adversarial CE term and the DANN λ schedule
    on top of the M12 relation-aux trainer. The GRL lives inside FARGCKNet, so
    the family CE is a normal positive term here; its gradient is reversed into
    the backbone automatically. λ ramps 0→1 across epochs (DANN), so the model
    starts at the M12 R011 solution and increases the family-invariance pressure
    as training proceeds. Set ``family_vocab``, ``family_adv_weight``,
    ``dann_gamma``, ``dann_max_epochs`` and ``fa_model`` after construction."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.family_vocab = {}
        self.family_adv_weight = 0.0
        self.dann_gamma = 10.0
        self.dann_max_epochs = 1
        self.fa_model = None
        self._dann_epoch = 0

    def _family_ce(self, logits, fam_strings):
        if fam_strings is None:
            return None
        idx = torch.tensor(
            [self.family_vocab.get(f, -1) for f in fam_strings],
            device=logits.device, dtype=torch.long,
        )
        mask = idx >= 0
        if not mask.any():
            return None
        return nn.functional.cross_entropy(logits[mask].float(), idx[mask])

    def _family_term(self, outputs, batch):
        """family_adv_weight · mean(CE_family_A, CE_family_B), or None."""
        if self.family_adv_weight <= 0.0 or not self.family_vocab:
            return None
        if not isinstance(outputs, tuple) or not isinstance(outputs[-1], dict):
            return None
        fam = outputs[-1]
        if "family_logits_a" not in fam:
            return None
        ce_a = self._family_ce(fam["family_logits_a"], batch.get("family1"))
        ce_b = self._family_ce(fam["family_logits_b"], batch.get("family2"))
        terms = [t for t in (ce_a, ce_b) if t is not None]
        if not terms:
            return None
        return self.family_adv_weight * (sum(terms) / len(terms))

    def train_epoch(self) -> float:
        import math
        from tqdm import tqdm

        # DANN schedule: λ = 2/(1+exp(-γ·p)) - 1, ramps 0→1 over training.
        p = min(1.0, self._dann_epoch / max(1, self.dann_max_epochs))
        lambd = 2.0 / (1.0 + math.exp(-self.dann_gamma * p)) - 1.0
        if self.fa_model is not None:
            self.fa_model.grl_lambda = float(lambd)
        self._dann_epoch += 1
        ...
        pbar = tqdm(self.train_loader, desc=f"Training (ROCm) λ={lambd:.3f}")
        for batch in pbar:
            img1 = batch["img1"].to(self.device, non_blocking=True)
            img2 = batch["img2"].to(self.device, non_blocking=True)
            labels = batch["label"].to(self.device, non_blocking=True)
...
def main() -> None:
    ...
    # M16: build the family vocabulary from the training split (each FIW family
    # is a domain for the adversary). Robust to a Subset-wrapped CV fold loader;
    # a superset of the fold's families is harmless (unused logits get no CE).
    from dataset import fiw_family_token  # noqa: E402
    _base_ds = getattr(train_loader.dataset, "dataset", train_loader.dataset)
    _pairs = getattr(_base_ds, "pairs", [])
    _fam = sorted({
        tok for pr in _pairs for k in (0, 1)
        for tok in (fiw_family_token(pr[k]),) if tok
    })
    family_vocab = {f: i for i, f in enumerate(_fam)}
    num_families = max(1, len(family_vocab))
    print(f"  [M16] family vocabulary: {num_families} training families "
          f"(adversary weight {args.family_adv_weight}, DANN γ {args.dann_gamma})")
    ...
    fa_model_ref = model  # M16: keep the FARGCKNet handle for the DANN λ schedule
    ...
    # M16: wire the family-adversarial objective + DANN λ schedule.
    ...
    model_config = {
        ...
        "family_adv_weight": float(args.family_adv_weight),
        "dann_gamma": float(args.dann_gamma),
