#!/usr/bin/env python3
"""
AMD ROCm evaluation script for Model 11 — AdaFace + FaCoR Cross-Attention.

Adapted from M02's evaluate.py. Image size + normalisation come from
checkpoint metadata; default AdaFace 112×112 + [-1, 1].
"""
import argparse
import json
import os
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Model 11 (AMD ROCm)")
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(min(len(labels), num_samples - sample_count)):
                ...
                plt.suptitle("AdaFace-FaCoR Cross-Attention Analysis (AMD ROCm)")
                plt.tight_layout()
...
def plot_roc_curve(predictions, labels, output_path):
    ...
    plt.title('ROC Curve — Model 11 (AdaFace-FaCoR, AMD ROCm)')
...
def main():
    ...
    print("\n" + "=" * 60)
    print("AMD ROCm Evaluation — Model 11: AdaFace + FaCoR")
    print("=" * 60)
    ...
    if args.full_analysis:
        ...
        plt.title('Confusion Matrix — Model 11 (AdaFace-FaCoR, AMD ROCm)')
