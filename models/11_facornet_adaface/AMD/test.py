#!/usr/bin/env python3
"""
AMD ROCm test script for Model 11 — AdaFace + FaCoR Cross-Attention.

Adapted from M02's test.py. Image size + normalisation come from the
checkpoint metadata; default to AdaFace 112×112 + [-1, 1].
"""
import argparse
import os
import sys
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test Model 11 (AMD ROCm)")
    ...
    parser.add_argument("--age_augment_root", type=str, default=None,
                        help="SAM aged-variants directory. Required if the "
                             "checkpoint was trained with age augmentation. If "
                             "unset and the checkpoint metadata recorded an "
                             "age_augment_root, that path is used.")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("AMD ROCm Testing — Model 11: AdaFace + FaCoR")
    print("=" * 60)
    ...
    if model_config.get("use_multistage"):
        ...
        print("  [M11] Rebuilding as top-only (M10 architecture)")
    ...
    # Resolve age-ensemble settings (CLI > checkpoint > off).
    ...
    use_age_ensemble = bool(age_root)
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing (ROCm)"):
            ...
            if args.visualize_attention and len(attention_samples) < args.num_visualizations:
                # For age-ensemble inputs, visualise only the original face.
                vis_img1 = img1[:, 0] if img1.dim() == 5 else img1
                vis_img2 = img2[:, 0] if img2.dim() == 5 else img2
                for i in range(min(len(vis_img1), args.num_visualizations - len(attention_samples))):
    ...
    with open(output_dir / "test_metrics_rocm.txt", "w") as f:
        f.write("AMD ROCm Test Results — Model 11 (AdaFace-FaCoR)\n")
        f.write("=" * 50 + "\n")
