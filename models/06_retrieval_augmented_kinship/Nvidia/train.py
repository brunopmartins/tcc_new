#!/usr/bin/env python3
"""Model 06 — Nvidia CUDA training (mirror of AMD/train.py)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Model 06 (Nvidia CUDA)")
...
class RetrievalLoss(nn.Module):
    def __init__(self, loss_type, contrastive_weight, relation_weight, temperature):
        super().__init__()
...
class RetrievalCUDATrainer(Trainer):
    def __init__(self, *args, gradient_accumulation=1, relation_to_index=None,
                 gallery_builder=None, gallery_refresh_every=0, **kwargs):
...
def main() -> None:
    ...
    train_config = TrainConfig(
        ...
        min_lr=args.min_lr, scheduler=args.scheduler, patience=args.patience,
        max_grad_norm=args.max_grad_norm, checkpoint_dir=args.checkpoint_dir,
        use_amp=not args.disable_amp, monitor_metric="roc_auc",
    )

    train_loader, val_loader, _ = create_dataloaders(
        config=train_data_config, batch_size=args.batch_size,
        dataset_type=args.train_dataset, split_seed=args.seed,
        num_workers=args.num_workers,
    )

    test_ds = KinshipPairDataset(
        root_dir=resolve_dataset_root(test_data_config, test_dataset_name),
        dataset_type=test_dataset_name, split="test",
        transform=get_transforms(test_data_config, train=False),
        split_seed=args.seed, negative_ratio=test_data_config.negative_ratio,
    )
    ...
    model = RetrievalAugmentedKinship(
        ...
        freeze_backbone=freeze, backbone_pretrained=not args.no_pretrained,
        embedding_dim=args.embedding_dim, retrieval_k=args.retrieval_k,
        retrieval_attn_layers=args.retrieval_attn_layers,
        retrieval_attn_heads=args.retrieval_attn_heads,
        dropout=args.dropout, relation_set=relation_set,
    ...
    print(f"total={total_params:,}  trainable={trainable:,} ({100*trainable/total_params:.2f}%)")

    gallery_ds = KinshipPairDataset(
        root_dir=resolve_dataset_root(train_data_config, args.train_dataset),
        dataset_type=args.train_dataset, split="train",
        transform=get_transforms(train_data_config, train=False),
        split_seed=args.seed, negative_ratio=0.0,
    )
    ...
    def build_gallery():
        ...
        print(f"[gallery] {n} pairs")

    build_gallery()

    loss_fn = RetrievalLoss(args.loss, args.contrastive_weight,
                            args.relation_loss_weight, args.temperature)

    trainer = RetrievalCUDATrainer(
    ...
    model_config = {
        "backbone_name": args.backbone_name, "img_size": args.img_size,
        "freeze_backbone": freeze, "embedding_dim": args.embedding_dim,
