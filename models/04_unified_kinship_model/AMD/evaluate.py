#!/usr/bin/env python3
"""
AMD ROCm Comprehensive evaluation script for Unified Kinship Model.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Unified Kinship Model (AMD ROCm)")
...
def run_component_ablation(model, dataloader, device, output_dir):
    ...
    plt.suptitle('Unified Model Component Analysis (AMD ROCm)', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_dir / "component_ablation_rocm.png", dpi=150)
    plt.close()
...
def cross_dataset_evaluation(model, device, data_config, batch_size, output_dir):
    ...
    if all("accuracy" in results[d] for d in results):
        ...
        ax.set_title('Cross-Dataset Evaluation (AMD ROCm)')
        ax.set_xticks(x)
        ...
        plt.tight_layout()
        plt.savefig(output_dir / "cross_dataset_rocm.png", dpi=150)
        plt.close()
...
def main():
    ...
    print("\n" + "=" * 60)
    print("AMD ROCm Evaluation - Unified Kinship Model")
    print("=" * 60)
    ...
    if args.full_analysis:
        ...
        plt.title('ROC Curve - Unified Model (AMD ROCm)')
        plt.legend()
        ...
        plt.title('Confusion Matrix - Unified Model (AMD ROCm)')
